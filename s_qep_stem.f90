...
! Precalculate the scattering factors on a grid
    ...
    if (.not.on_the_fly) then
        ...
        !DMH if(.not. load_grates) call load_save_add_grates(idum,projected_potential,nopiy,nopix,n_qep_grates,n_slices,nt,nat_slice)
...
900     format(1h+,1x,' df:',i3,'/',i3,' y:',i3,'/',i3,' x:',i3,'/',i3,'  Intensity:', f6.3, ' (to monitor BWL)')	
    ...
    if(ionization) then
		...
		!DMH if(.not.EDX) eels_correction_image = eels_correction_image/float(n_qep_passes)
                if(.not.EDX) then
                   eels_correction_image = eels_correction_image/float(n_qep_passes)
                endif
	endif
